...
{
    auto timer_it = std::find_if(_timer_handlers.begin(), _timer_handlers.end(), [timer_id](timer_t const &timer) {
...
loop_t::time_point_t loop_t::now()
{
...
loop_t::time_milliseconds_t loop_t::find_next_timeout(time_point_t const &actual_time)
{
    auto const next_expiring_timer_it =
        std::min_element(_timer_handlers.cbegin(), _timer_handlers.cend(), [](timer_t const &a, timer_t const &b) {
